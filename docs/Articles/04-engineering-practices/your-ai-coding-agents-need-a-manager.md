---
title: 260307-你的 AI Coding Agents 需要一个管理者
description: 在不久的将来，高杠杆开发者会更像异步优先的管理者，负责并行编排多位 AI Coding Agents。那些让人成为优秀技术负责人或管理者的能力，也会直接转化为新的 AI 协作能力。
author: Addy Osmani
...
**那些让人成为优秀技术负责人或管理者的能力，正在直接转化为新的 AI 协作能力。** 一旦你开始并行运行多个 Coding Agents，你做的就不再只是“补上下文、改提示词”，而是在做更明确的编排工作：讲清目标、拆分任务、建立验证循环，并处理异步沟通。随着[编排](https://addyosmani.com/blog/future-agentic-coding/)成为主流，工作的形态也在改变。

在不久的将来，我认为最高杠杆的开发者会更像**异步优先的管理者**，手里同时跑着一小队并行的 AI Coding Agents。它们已经可以在后台、隔离环境里完成有意义的工作，再把结果作为可审查的产物交回来——通常就是一个 PR。我们当然仍然需要质量把关，但真正的瓶颈已经转向“这件事值不值得做”以及“我能不能高效地编排多个智能体同时推进”。

我们正在看到预示这一未来的[工作流](https://code.claude.com/docs/zh-CN/common-workflows)。Claude Code 创建者 Boris Cherny 最近的一个[帖子](https://twitter-thread.com/t/2007179832300581177)走红了，因为它让这种转变感觉具体：他在终端标签页中本地运行五个 Claude Code 会话，在浏览器中再运行五到十个，甚至从手机上启动会话以便稍后查看。这就是编排。

与此同时，也有一些工程师从更审慎的角度记录这种并行代理工作流。Simon Willison 的[观点](https://simonwillison.net/2025/Oct/5/parallel-coding-agents/)我很认同：真正的瓶颈不是生成代码，而是审查代码。他仍然认为并行启动任务很有价值，但前提是你得诚实面对自己的注意力极限，并挑选那些不会把自己拖进认知过载的任务。我自己的体验也类似：价值是真实的，但前提是你把它当作管理问题，而不是魔法。
...
GitHub 的 Copilot Coding Agent 也是类似定位——一个异步后台智能体，会先打开草稿 PR，在后台推进任务，完成后再请求审查，你可以在 PR 中继续评论并让它迭代。平台也在朝着“多智能体任务控制台”的方向演进，而不再只是一个单点助手。GitHub 甚至预览了 “Agent HQ”，作为协调多家第三方 Coding Agents 的统一控制平面，包括在同一任务上并行运行它们并比较输出。
...
其次，把持久规则放在智能体总能找到的地方。现在比较常见的做法，是通过 `AGENTS.md` 明确测试要求、lint 规则、依赖策略和文档约定，让执行预期保持一致。
...
如果你同时运行十个并行会话，就不可能靠逐个滚动日志去理解发生了什么。你需要的是轻量、稳定、可复用的状态汇报机制——这本质上就是一套面向智能体的异步管理手册。
...
像管理者一样规划——用结果、约束和验收标准写简报。像编排者一样执行——把任务分配给边界清晰的并行智能体。异步监控——通过轻量状态同步快速解除阻碍，避免并行工作失控。积极验证——跑测试、过 lint、整理结构化 PR 包，并在有价值时增加第二个智能体做复审。谨慎集成——按深思熟虑的顺序合并，并留意边界冲突。最后做复盘——更新你的 [AGENTS.md](https://developers.openai.com/codex/guides/agents-md/) 和检查清单，让下一轮运行从更好的约束条件出发。
