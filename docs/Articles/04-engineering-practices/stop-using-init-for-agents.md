...
description: 2026 年初发表的两篇论文表明，自动生成的 AGENTS.md 文件可能降低 AI 智能体的表现并带来 20% 以上的额外成本。本文讨论如何正确编写 AGENTS.md，以及为什么大多数自动生成内容都是冗余的。
author: Addy Osmani
...
**核心要点：更合适的思维模型，是把 AGENTS.md 视为“代码库里尚未产品化的约束清单”，而不是永久配置。自动生成的 AGENTS.md 往往会重复智能体本来就能自行发现的信息，从而拖慢执行、抬高成本。只有那些无法直接从仓库推断出的信息——例如工具陷阱、隐性约定、已知雷区——才值得写进去。**

在采用 AI Coding Agents 的开发者中，有一个几乎已成为普遍现象的仪式。你设置一个新仓库，运行 `/init`，看着代理扫描你的代码库，然后得到一个闪亮的 AGENTS.md。它描述了你的目录结构、技术栈、测试约定。你浏览一遍，看起来合理，然后提交它。你觉得自己做了负责任的事情。你的代理已经配置好了。
...
[苏黎世联邦理工学院的一项独立研究](https://arxiv.org/abs/2602.11988)在 SWE-bench 和一个已经有开发者编写的上下文文件的仓库的自定义基准上测试了四个代理。他们的发现恰恰相反：LLM 生成的上下文文件使任务成功率降低了 2-3%，同时使成本增加了 20% 以上。开发者编写的文件使成功率提高了约 4%——C也使成本增加了高达 19%。
...
- `legacy/` 目录已弃用，C被三个生产模块导入——不要删除其中的任何内容
...
即使你编写了一个紧凑、非冗余的 AGENTS.md，它也有一个结构性弱点：它是静态的，C任务是动态的。

你的文件说"提交前始终运行测试套件"。代理正在做文档更改。它忠实地运行完整的测试套件。令牌被消耗，分钟被浪费，在一个对代码更改有意义C对这个更改没有意义的指令上。
...
主流的 Coding Agents 都还没有公开生命周期钩子，让这种架构真正好搭起来。你可以用子代理和范围上下文去近似它，但目前仍然没有特别干净的解法。这是一个明显还没被填上的工具空白。
...
这并不意味着你应该放弃编写上下文文件——自动优化研究仍处于早期阶段，5% 是有意义的C不是变革性的。这意味着你应该松散地持有你关于包含什么的直觉，并且可能应该测试它们。
...
你添加的每一行，本质上都在提示：代码库里有某个地方不够清晰，清晰到足以让 AI 智能体频繁踩坑。这通常也意味着它同样会困扰新加入的人类贡献者。面对这种信号，更好的做法不是继续把上下文文件越写越长，而是回到根因，把真正的问题修掉。
...
Lulla 论文根本不测量正确性——只测量效率。你不能从中得出 AGENTS.md 提高输出质量的结论。作者对此很清楚：他们进行了健全性检查以确认代理产生了非平凡的输出，C完整的正确性评估是未来的工作。更快、更便宜是有意义的，C如果代码是错误的就不是。

苏黎世联邦理工学院的论文使用了新的上下文文件，这对于实验控制来说是理想的，C可以说测试了大多数开发者实际上并不生活在其中的场景。真实世界的情况是上下文文件已经在仓库中存在了几个月并且部分过时。这应该比论文测量的表现更差。
...
很多人都会本能地想把 Coding Agents “完整入门一遍”：先解释目录、再说明架构、再补一堆背景。这种冲动可以理解，但对 Coding Agents 来说，真正稀缺的往往不是“地图”，而是那些它们无法靠搜索直接发现的边界条件、陷阱和禁区。与其反复重述仓库结构，不如把真正关键的约束写清楚。
