...
```
#### 7.1 .env 文件配置 (./01-env文件配置.md)
环境变量的基本规则、命名规范、配置方法,以及常见错误排查。

#### 7.2 Server vs Client 详解 (./02-Server-vs-Client详解.md)
服务端和客户端的区别、环境变量访问规则、渲染方式对比。

#### 7.3 服务端渲染 vs 客户端渲染 (./03-服务端渲染-vs-客户端渲染.md)
SSR、CSR、SSG 的区别,Hydration 过程,性能优化策略。

#### 7.4 .gitignore 最佳实践 (./04-gitignore最佳实践.md)
.gitignore 文件的作用、语法、安全检查清单,以及敏感文件处理。

#### 7.5 环境变量同步到云端 (./05-环境变量同步到云端.md)
如何在 Vercel、Railway、Docker 等平台配置环境变量,自动化部署流程。

#### 7.6 Middleware 路由保护 (./06-Middleware路由保护.md) 🔴
使用 Next.js Middleware 实现路由保护、认证授权、速率限制等功能。

#### 7.7 安全检查清单 (./07-安全检查清单.md)
完整的安全检查清单,涵盖环境变量、Git、认证、路由、数据、传输、依赖、日志、部署等各个方面。

#### 7.8 进阶安全防护 (./08-进阶安全防护.md) 🟡
