...
| **第三部分：细节方案** | 9稿 | 交互说明、边缘情况、非功能需求 |
| **第四部分：上线计划** | 9稿 | 排期、灰度发布 |

**一五九稿原则**：1稿想清楚"为什么"，5稿想清楚"是什么"，9稿想清楚"怎么做"。每一步都有评审和修改，避免到最后才发现大问题。

这个迭代节奏有其心理学基础。人在面对复杂问题时，往往会陷入"分析瘫痪"——想要一次性想清楚所有细节，结果反而寸步难行。"一五九稿"原则通过强制分阶段，让你在每个阶段只关注特定的维度。1稿时，你不需要担心技术实现，只需要确认方向正确；5稿时，你不需要纠结按钮的颜色，只需要确认流程合理。这种分层的思考方式，让复杂问题的处理变得可管理。

另一个容易被忽视的好处是，每一稿都是一个"可回退的检查点"。如果在5稿时发现方向有问题，你可以回到1稿重新审视；如果在9稿时发现技术方案不可行，你可以回到5稿调整流程。这种结构化的迭代，让错误可以在早期被发现和纠正，而不是累积到最后爆发。
...
范围管理应该在 3.2 与 AI 确认需求时完成。这里只是把讨论结果记录下来。
...
**A**: 1稿可以简略，5稿要有流程图，9稿要包含边缘情况。原则是：AI 读了之后，不需要再问"这个按钮放哪"、"失败时怎么处理"这类问题。
