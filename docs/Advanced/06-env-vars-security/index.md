...
**第一层：基础安全**（本章重点）。环境变量、.gitignore、路由保护。这是入门必修课，不做这些等于裸奔。

**第二层：应用安全**（本节进阶安全防护）。输入验证、SQL注入防护、XSS防护。这是开发中必须注意的，每个功能都要考虑安全边界。

**第三层：深度安全**。漏洞扫描、渗透测试、安全审计。这是进阶内容，当你产品上线、有真实用户时，需要定期做。
...
基础安全还不够。随着产品上线，你需要面对更复杂的安全威胁。

**SQL 注入防护**：永远不要拼接 SQL 字符串。使用参数化查询，或使用 ORM（如 Prisma）。Prisma 会自动处理参数转义，防止注入攻击。

**XSS 攻击防护**：输入验证——所有用户输入都要验证和清理。使用 CSP（Content Security Policy）——在 Next.js 中配置 CSP 头，限制外部资源加载。React 默认转义输出，这是天然防护。

**CSRF 攻击防护**：使用 Token 验证——Next.js 的内置 CSRF 保护。验证 Referer 头——确保请求来自你的网站。

**敏感数据泄露**：环境变量存储密钥（已在前面讲过）。审计日志——记录谁访问了什么数据。加密存储——数据库敏感字段加密。

**工具辅助**：使用 Prisma ORM 避免 SQL 注入。Next.js 内置 XSS 防护。定期运行 `npm audit` 检查依赖漏洞。使用 Snyk 扫描安全漏洞。
