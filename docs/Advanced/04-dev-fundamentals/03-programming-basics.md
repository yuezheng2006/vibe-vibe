...
用伪代码描述需求时，AI 可能给出不同的实现方案。如何判断哪个方案更好？这就需要理解算法思维——评估代码效率的能力。

这里有一个经典例子：**二分查找**。

想象你在电话簿里查找 "Mike Smith"。最笨的方法是从第一页开始逐页翻——这叫**线性查找**。如果电话簿有 1000 页，最坏情况下你要翻 1000 页。

更聪明的方法是：先翻到中间，看当前页是 M 开头——Mike 在字母表中排在 M 后面，所以前半本可以直接扔掉。然后对剩下的半本重复这个过程。这就是**二分查找**：每次把问题规模减半。1000 页的电话簿，最多只需要翻 10 次。

这个例子说明了一个重要概念：**算法的选择会极大影响程序性能**。当你让 AI 生成代码时，如果数据量很大，可以提醒它"使用更高效的查找方法"。
