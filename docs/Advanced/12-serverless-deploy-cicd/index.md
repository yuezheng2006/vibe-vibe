...
小明的"个人豆瓣"代码已经推到 GitHub 上了，还和朋友通过分支和 PR 协作了几次。一切看起来很顺利——直到朋友发来消息："你上次用 Tunnel 给我看的那个链接打不开了，能不能弄个一直能用的？我想发给我女朋友看看。"

小明一愣。第十章用 Cloudflare Tunnel 临时分享的链接，关了终端就失效了。他需要一个**永久在线**的地址——任何人、任何时候都能打开。
...
3. **服务端缓存**：服务器内存里缓存计算结果或数据库查询结果，避免重复计算。你可能注意过，第一次打开某个页面比较慢，之后再打开就快了——即使清了浏览器缓存也是如此，这往往就是服务端缓存在起作用。
...
AI模型成本也值得优化。简单任务用便宜的国产模型（如GLM、DeepSeek），复杂推理用Claude。你可以在代码里加一层判断：简单的翻译、摘要请求发给便宜模型，需要深度推理的请求才发给 Claude——这就是"智能路由"。某AI应用通过智能路由（70% GLM + 30% Claude），API成本降低了80%。关键是匹配任务复杂度和模型能力，不要用大炮打蚊子。

老师傅还提醒你注意**免费额度陷阱**：很多平台的免费额度只够测试用，一旦有真实用户，费用会指数级增长。记得设置账单告警（比如"超过¥70发邮件提醒"），定期查看用量，做到心中有数。免费额度是用来体验的，不是用来长期跑生产业务的。

至于运维监控、日志分析等SRE相关的技术实践，我们会在后续云服务器运维中详细讲解。

## 本章学习路径

1. [部署到 EdgeOne Pages](./01-deploy-edgeone.md) → 手把手完成第一次部署，拿到公网链接
2. [部署到类 Vercel 平台](./02-deploy-vercel-platforms.md) → 了解更多平台选择，找到最适合你的
3. [CI/CD 与自动化](./03-cicd-automation.md) → 推送即部署，让自动化接管重复劳动
4. [运维基础与成本优化](./04-operations-cost.md) → 上线之后谁来看着它，怎么控制成本
