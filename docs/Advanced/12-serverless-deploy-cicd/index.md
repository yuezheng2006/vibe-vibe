...
网站上线后，你肯定会继续迭代功能。老师傅告诉你，更新网站非常简单——只需要在本地修改代码，然后 `git push`，部署平台就会自动检测到代码变更并重新部署。

但有一个重要细节需要注意：**Schema 变更需要推送**。如果你修改了数据库表结构（比如在 `schema.ts` 里加了一个新字段），不仅代码要推送到 GitHub，还需要在本地运行一次 `npx drizzle-kit push`，把新的表结构同步到数据库。这个命令会把你的 Schema 定义和数据库的实际结构做对比，然后只推送差异部分。

新手容易忘记这一步。如果修改了 Schema 但忘记了 `push`，网页可能会报错 `column "xxx" does not exist`。这时候别慌，运行一下 `drizzle-kit push` 就好了。
